package roguelike.model.game.elements;

import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.BeforeEach;
import org.junit.jupiter.api.Test;
import org.mockito.Mockito;
import roguelike.model.Position;

class MobTest {

    class GenericMob extends Mob {
        public GenericMob(Position position, int health, int strength) { super(position, health, strength); }
    }

    GenericMob mob;

    @BeforeEach
    public void createMob() {

        Position pos = Mockito.mock(Position.class);

        mob = new GenericMob(pos, 10, 7);
    }

    @Test
    public void initialStrengthTest() {
        Assertions.assertEquals(mob.strength, mob.initialStrength);
        Assertions.assertEquals(7, mob.initialStrength);
    }

    @Test
    public void increaseHealthTest() {
        Assertions.assertEquals(10, mob.health);

        mob.increaseHealth(15);
        Assertions.assertEquals(25, mob.health);
        mob.increaseHealth(7);
        Assertions.assertEquals(32, mob.health);
        mob.increaseHealth(0);
        Assertions.assertEquals(32, mob.health);
        mob.increaseHealth(-8);
        Assertions.assertEquals(32, mob.health);
    }

    @Test
    public void increaseStrengthTest() {
        Assertions.assertEquals(7, mob.strength);

        mob.increaseStrength(10);
        Assertions.assertEquals(17, mob.strength);
        mob.increaseStrength(0);
        Assertions.assertEquals(17, mob.strength);
        mob.increaseStrength(-7);
        Assertions.assertEquals(17, mob.strength);
    }

    @Test
    public void increaseStrengthBonusTimeTest() {
        Assertions.assertEquals(0, mob.strengthBonusTime);

        mob.increaseStrengthBonusTime(12);
        Assertions.assertEquals(12, mob.strengthBonusTime);
        mob.increaseStrengthBonusTime(0);
        Assertions.assertEquals(12, mob.strengthBonusTime);
        mob.increaseStrengthBonusTime(-8);
        Assertions.assertEquals(12, mob.strengthBonusTime);
    }

    @Test
    public void decreaseHealthTest() {
        Assertions.assertEquals(10, mob.health);

        mob.decreaseHealth(7);
        Assertions.assertEquals(3, mob.health);
        mob.decreaseHealth(0);
        Assertions.assertEquals(3, mob.health);
        mob.decreaseHealth(-8);
        Assertions.assertEquals(3, mob.health);
        mob.decreaseHealth(8);
        Assertions.assertEquals(0, mob.health);
        mob.decreaseHealth(5);
        Assertions.assertEquals(0, mob.health);
    }

    @Test
    public void decreaseStrengthBonusTimeTest() {
        Assertions.assertEquals(0, mob.strengthBonusTime);

        mob.strengthBonusTime = 5;

        mob.decreaseStrengthBonusTime();
        Assertions.assertEquals(4, mob.strengthBonusTime);
        mob.decreaseStrengthBonusTime();
        Assertions.assertEquals(3, mob.strengthBonusTime);
        mob.decreaseStrengthBonusTime();
        Assertions.assertEquals(2, mob.strengthBonusTime);
        mob.decreaseStrengthBonusTime();
        Assertions.assertEquals(1, mob.strengthBonusTime);
        mob.decreaseStrengthBonusTime();
        Assertions.assertEquals(0, mob.strengthBonusTime);
        mob.decreaseStrengthBonusTime();
        Assertions.assertEquals(0, mob.strengthBonusTime);
    }

}